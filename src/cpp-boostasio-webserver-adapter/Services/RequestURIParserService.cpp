...
void RequestURIParserService::parseQueryString(Request& request) const {
  ...
  if (requestURI.find("?") != std::string::npos) {
    std::string requestURIWithoutQueryStrings =
        requestURI.substr(0, requestURI.find("?"));
    request.setURI(requestURIWithoutQueryStrings);

    std::string queryStrings =
        requestURI.substr(requestURIWithoutQueryStrings.size());
    std::regex pattern("([\\w+%]+)=([^&]*)");
    auto words_begin =
        std::sregex_iterator(queryStrings.begin(), queryStrings.end(), pattern);

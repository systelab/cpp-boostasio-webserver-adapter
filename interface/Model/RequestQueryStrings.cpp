...
RequestQueryStrings::RequestQueryStrings() : m_items() {}
...
std::string RequestQueryStrings::getItem(const std::string& name) const {
  ...
  if (m_items.find(name) != m_items.end()) {
    ...
    std::string exc =
        std::string("Query string item '") + name + std::string("' not found.");
    throw std::exception();
  }
}

void RequestQueryStrings::addItem(const std::string& name,
